...
var getChild = function getChild(childData) {
	var Comp = (0, _ControlType.getComponentByType)(childData.options.type);
	return Comp ? (0, _preact.h)(Comp, _extends({ name: childData.name }, childData.options, { content: childData.content, origin: childData.content })) : null;
};
...
var Base = function (_Component) {
	...
	_createClass(Base, [{
		...
		value: function getControlLabel() {
			return this.props.label || varNameToLabel(this.props.name);
		}
	}], [{
		key: 'varNameToLabel',
		value: function varNameToLabel(name) {
			return (0, _utils.fieldNameToLabel)(name);
		}
	}]);
...
var fieldNameToLabel = exports.fieldNameToLabel = function fieldNameToLabel(name) {
  return name.split(/(?=[A-Z\-_])/).map(function (s) {
    return s.replace(/[\-_]/, '');
  }).toLowerCase();
};
...
var Input = (_class = function (_Base) {
	...
	_createClass(Input, [{
		...
		value: function onChange(evt, overrideCallback) {
			...
			// TODO: optimize, move type check to constructor
			var ptrn = this.props.pattern;
			if (ptrn) {
				if (typeof ptrn === 'function') {
					if (!ptrn(newValue)) return evt.preventDefault();
				} else if (typeof ptrn === 'string') {
					var exp = new RegExp(ptrn);
					if (!exp.test(newValue)) return evt.preventDefault();
				} else if (ptrn instanceof RegExp) {
					if (!ptrn.test(newValue)) return evt.preventDefault();
				}
			}
		...
		value: function render(_ref, state) {
			...
			var labelText = label || this.parsePropName(name);
			return (0, _preact.h)(
				'div',
				{ 'class': 'inputControl' },
				(0, _preact.h)(
